...
export const DashboardPanel: React.FC = () => {
  ...
  const [newRefreshSec, setNewRefreshSec] = useState(10);
  ...
  const handleCreateWidget = (e: React.FormEvent) => {
    ...
    setNewTitle('');
    showToast('ویجت جدید با موفقیت به داشبورد افزوده شد!');
  };
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center bg-[#101625] border border-[#1e2942] rounded-lg px-2 py-1 text-xs">
            ...
            >
              ...
              <option value={60} className="bg-[#101625]">Every 1m</option>
            </select>
          </div>
      ...
      <div className="flex-1 p-5 overflow-y-auto">
        {widgets.length === 0 ? (
          <div className="h-64 flex flex-col items-center justify-center text-slate-500 text-center">
            ...
            <span className="text-sm font-semibold text-slate-300">داشبورد زنده خالی است</span>
            <span className="text-xs text-slate-500 mt-1 max-w-sm">
              برای ساخت اولین ویجت مدیریتی روی دکمه "Add Widget" کلیک کنید یا نتایج کوئری‌ها را پین کنید.
            </span>
            <button
              onClick={() => setAddModalOpen(true)}
            ...
            >
              ...
              <span>ساخت اولین ویجت داشبورد</span>
            </button>
          </div>
        ) : (
          ...
          <div className="w-full max-w-md bg-[#0d121c] border border-[#1c263c] rounded-xl shadow-2xl p-5 text-xs text-slate-200 space-y-4 animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between pb-3 border-b border-[#1c263c]">
              <div className="flex items-center gap-2 font-bold text-sm text-white">
                ...
                <span>افزودن ویجت جدید به داشبورد</span>
              </div>
            ...
            <form onSubmit={handleCreateWidget} className="space-y-3">
              <div>
                <label className="block text-[11px] font-medium text-slate-400 mb-1">عنوان ویجت</label>
                <input
                  type="text"
                  value={newTitle}
                  onChange={(e) => setNewTitle(e.target.value)}
                  placeholder="مثال: مجموع ردیف‌های جدول کاربران"
                  required
              ...
              <div>
                <label className="block text-[11px] font-medium text-slate-400 mb-1">دستور SQL ویجت</label>
                <textarea
                  value={newSql}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-medium text-slate-400 mb-1">نوع چارت / نمایش</label>
                  <select
                    value={newChartType}
                ...
                <div>
                  <label className="block text-[11px] font-medium text-slate-400 mb-1">نرخ به‌روزرسانی</label>
                  <select
                    value={newRefreshSec}
                    onChange={(e) => setNewRefreshSec(Number(e.target.value))}
                    className="w-full bg-[#111726] border border-[#232e48] rounded-lg px-2.5 py-2 text-xs text-slate-200 focus:outline-none"
                  >
                    <option value={0}>Manual (دستی)</option>
